...
class RegressFlow(nn.Module):
    ...
    def forward(self, pred_jts, sigma, target_jts):
        # import pudb;pudb.set_trace()
        N, num_kps, _ = pred_jts.shape
        assert num_kps == self.num_joints
        if target_jts is not None:
            gt_uv = target_jts.reshape(pred_jts.shape)
            bar_mu = (pred_jts - gt_uv) / sigma
            # (B, K, 2)
            
            log_phi = self.flow.log_prob(bar_mu.reshape(-1, 2)).reshape(N, num_kps, 1)
...
class RLELoss(nn.Module):
    ...
    def forward(self, pred_jts, sigma, nf_loss, target_jts, target_jts_weight):
        
        gt_uv = target_jts.reshape(pred_jts.shape)
        gt_uv_weight = target_jts_weight.reshape(pred_jts.shape)
        # print(gt_uv_weight)
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        self.flow = RegressFlow()
        self.RLE_loss = RLELoss(size_average=True)
        self.aux_heatmap_loss = FocalLoss()
    ...
    def loss_kps(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, w, h), normalized by the image size.
        """
        ...
        if target_kps.shape[0] != 0:
            ####################### contrust the relative keypoint label ##############
            # image_size = torch.cat([v["image_size_xyxy_tgt"] for v in targets])
            offset_x = src_boxes[:, 0:1]
            offset_y = src_boxes[:, 1:2] 
            offset = torch.cat([offset_x, offset_y], dim=1) # n,2
            roi_w = src_boxes[:, 2:3]- src_boxes[:, 0:1] # n,1
            roi_h = src_boxes[:, 3:4]- src_boxes[:, 1:2] # n,1
            rel_kps = target_kps - offset.unsqueeze(1).repeat(1,17,1)
            target_vis[rel_kps[:,:,0]<0] = 0
            target_vis[rel_kps[:,:,0]>roi_w] = 0
            target_vis[rel_kps[:,:,1]<0] = 0
            target_vis[rel_kps[:,:,1]>roi_h] = 0
            target_vis = (target_vis > 0).float().repeat(1,1,2)

            rel_kps[:, :, 0] = rel_kps[:, :, 0] / roi_w - 0.5
            rel_kps[:, :, 1] = rel_kps[:, :, 1] / roi_h - 0.5
            #####################################################
        
            assert src_kps[:,:,:-1].shape == src_sgm.shape == rel_kps.shape == target_vis.shape
            
            nf_loss = self.flow(src_kps[:,:,:-1], src_sgm, rel_kps)
            
            loss_kps = self.RLE_loss(src_kps[:,:,:-1], src_sgm, nf_loss, rel_kps, target_vis)
            loss_kps = loss_kps.sum() / (num_boxes * 17)
       
        else:
            # placeholder
            loss_kps = F.l1_loss(src_kps[:,:,:-1], target_kps, reduction='none')
            loss_kps = loss_kps.sum()/num_boxes
        losses = {}
        losses['loss_kps'] = loss_kps 
    ...
    def forward(self, outputs, heatmap, targets):
        ...
        if heatmap is not None:
            losses['loss_aux_hm'] = self.aux_heatmap_loss(heatmap, torch.stack([t['heatmap'] for t in targets], dim=0))
        return losses
