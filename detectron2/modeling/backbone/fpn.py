...
class HRFPN(Backbone):
    ...
    def forward(self, x):
        ...
        return dict(list(zip(self._out_features, outputs)))



class HRFPN_v1(Backbone):
    ...
    def __init__(self, 
        ...
        if self.share_conv:
            ...
            self.fpn_conv = nn.ModuleList()
            use_bias = normalize is None
            for idx, in_channels in enumerate(self.in_channels):
                lateral_norm = get_norm(normalize, out_channels)
                output_norm = get_norm(normalize, out_channels)
                ...
                self.fpn_conv.append(output_conv)
        #########################################################
            output_norm = get_norm(normalize, out_channels)
            output_conv = Conv2d(
                    out_channels,
    ...
    def forward(self, x):
        ...
        # for i in range(1, 5):
        #     # out = self.pooling(out, kernel_size=2**i, stride=2**i)
        ...
        if self.share_conv:
            for i in range(4):
                outputs.append(self.fpn_conv(out))
        else:
            # print("HRNET_FPN_V1")
            for i in range(5):
                if i==0:
                    tmp_out = self.fpn_conv[i](out)
                elif i<4:
                    out = self.pooling(out, kernel_size=2, stride=2) + self.lateral_conv[i](inputs[i])
                    tmp_out = self.fpn_conv[i](out)
                else:
                    out = self.pooling(out, kernel_size=2, stride=2)
                    tmp_out = self.fpn_conv[i](out)
                outputs.append(tmp_out)
        assert len(self._out_features) == len(outputs[:-1])
        return dict(list(zip(self._out_features, outputs[:-1])))
...
def build_hrnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    in_features = cfg.MODEL.HRNET.FPN_IN
    out_channels = cfg.MODEL.HRNET.FPN_OUT
    backbone = HRFPN(bottom_up, in_features, out_channels,
                     normalize=cfg.MODEL.FPN.NORM
                  )
    return backbone

@BACKBONE_REGISTRY.register()
def build_hrnet_fpnv1_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
    ...
    in_features = cfg.MODEL.HRNET.FPN_IN
    out_channels = cfg.MODEL.HRNET.FPN_OUT
    backbone = HRFPN_v1(bottom_up, in_features, out_channels,
                     normalize=cfg.MODEL.FPN.NORM
                  )
    return backbone
